...
(** [subst e v x] is [e] with the value [v] substituted in for the name/variable
    [x] *)
let rec subst e v x =
  match e with
  | Int _ | Bool _ -> e
...
let rec eval (e : 'a expr) : 'a expr =
  match e with
  | Int _ | Bool _ | Closure _ -> e
  | Var x ->
      failwith
        ("unbound variable " ^ x ^ " while evaluating " ^ string_of_expr e)
