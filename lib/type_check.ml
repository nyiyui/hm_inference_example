...
type tConType = TClosure | TApplication
(* TList etc... *)

(* use variable names: t, t1 *)
type typ = TVar of string | TCon of tConType * typ list | TKind | TInt | TBool
...
let rec not_contains (a : string) (t : typ) : bool =
  match t with
...
let vars_in (t : typ) : string list =
  let rec vars_in' (acc : string list) (t : typ) : string list =
    match t with
    | TVar x -> x :: acc
...
let rec apply_typ (s : subst) (t : typ) : typ =
  match t with
  | TVar x -> ( try List.assoc x s with Not_found -> t)
  | TCon (c, ts) -> TCon (c, List.map (apply_typ s) ts)
  | _ -> t

(** [apply_typ (compose s1 s2) t] is equivalent to
    [apply_typ s2 (apply_typ s1 t)]. *)
let compose (s1 : subst) (s2 : subst) : subst =
  ...
  let s2' =
    List.filter
      (fun (x, _) -> List.find_opt (fun (y, _) -> x = y) s1' = None)
      s2
  in
  s1' @ s2'
...
let generalize (env : Env.t) (t : typ) : typ_scheme =
  ...
  let vars_not_in =
    List.filter (fun x -> not (List.mem x vars_in_env)) vars_in_t
  in
  (vars_not_in, t)
...
let rec unify (t1 : typ) (t2 : typ) : subst =
  ...
  | _ ->
      failwith
        ("unification failed between " ^ string_of_typ t1 ^ " and "
       ^ string_of_typ t2)
...
let rec infer (e : 'i expr) (env : Env.t) : typ * subst =
  let unary_expect s t = function
    | TVar x ->
        ...
        (t, compose s s')
    | t' when t' = t -> (t', s)
    | _ -> failwith ("must be " ^ string_of_typ t)
  in
  let binary_expect t e1 e2 env =
    ...
    | _ ->
        failwith
          ("both sides must be " ^ string_of_typ t ^ " but lhs is "
         ^ string_of_typ t1 ^ " and rhs is " ^ string_of_typ t2)
  in
  match e with
  | Var x ->
  ...
  | Application (e1, e2) ->
      ...
      (* LHS side might have useful info (e.g. the func only takes in int, or `(plus1 x) (id x)' ) *)
      let t2, s2 = infer e2 env1 in
      (* right hand side *)
      let t1' = apply_typ s2 t1 in
